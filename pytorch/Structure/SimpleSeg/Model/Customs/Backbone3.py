...
class DecoderBlock(torch.nn.Module):
    def __init__(self,n_resblocks,n_subblocks,input_channels,output_channels):
        super().__init__()
        self.body = torch.nn.Sequential(
            AttentionBlock(n_resblocks,n_subblocks,input_channels,2),
            torch.nn.Conv2d(input_channels,output_channels,1,bias=False),
            torch.nn.InstanceNorm2d(output_channels, eps=1e-05, momentum=0.1, affine=False, track_running_stats=False),
            torch.nn.Mish(inplace=True))
...
class Decoder(torch.nn.Module):
    def __init__(self,output_channels):
        ...
        self.upsample6 = DecoderBlock(2,1,16+16,output_channels) #512
